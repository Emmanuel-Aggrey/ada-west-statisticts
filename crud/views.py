...
from .models import Member, Document, Message, CsvUpload
import datetime
...
def ajax(request):
    if request.method == 'POST':
        if request.is_ajax():
            ...
            deliver_at=request.POST['deliver_at'],
        ...
        ajax_list = Message.objects.order_by('-created_at')
        context = {'ajax_list': ajax_list}
    return render(request, 'message.html',  {'ajax_list': ajax_list})

@csrf_protect
def getajax(request):
    if request.method == 'GET':
        if request.is_ajax():
            data = Message.objects.order_by('-created_at').first()
...
def ajax_delete(request):
    if request.method == 'GET':
        if request.is_ajax():
            id=request.GET['id']
            ajax = Message.objects.get(id=id)
            ajax.delete()
            return JsonResponse({'data': 'success'})
    else:
        return JsonResponse({'data': 'failure'})
